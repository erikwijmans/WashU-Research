...
void satoshiRansacManhattan2(const std::vector<Eigen::Vector3d> &,
                             const Vector3d &, Vector3d &, Vector3d &);
...
void analyzeNormals(const string &normalsFileName, const string &outputFolder) {
  ...
  const string number =
      normalsFileName.substr(normalsFileName.find(".") - 3, 3);
  const string buildName =
      normalsFileName.substr(normalsFileName.rfind("/") + 1, 3);
  const string rotOut =
      outputFolder + buildName + "_rotations_" + number + ".dat";
...
void satoshiRansacManhattan2(const std::vector<Eigen::Vector3d> &N,
                             const Vector3d &n1, Vector3d &M1, Vector3d &M2) {
  const int m = N.size();
...
#pragma omp parallel shared(k, K, maxInliers, N, n1, M1, M2)
  {
    ...
    while (k < K) {
      ...
      Eigen::Vector3d average = Vector3d::Zero(), average2 = Vector3d::Zero();
      for (auto &n : N) {
