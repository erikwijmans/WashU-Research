...
cv::Vec3d apply_blur(const cv::Mat3b &img, int x, int y, double sigma) {
  ...
  for (auto && [ x, y, width, height ] : rects) {
    ...
    for (int j = 0; j < iheight; ++j) {
      for (int i = 0; i < iwidth; ++i) {
        ...
        res(yprime, xprime) = apply_blur(
            img, xprime, yprime,
            4.0 * std::exp(1) *
                std::exp(
                    -(xprime - x_mid) * (xprime - x_mid) / (iwidth * iwidth) -
...
int main(int argc, char const *argv[]) {
  ...
  for (auto &data_folder : utils::parse_folder(rects_folder)) {
    ...
    for (auto &ptx_file : utils::parse_folder(ptx_folder)) {
      const auto[building_name, scan_id] = parse_name(ptx_file.string());
      ...
      if (fs::exists(release_ptx_file) && fs::exists(release_img_file))
        continue;
      ...
      std::map<std::tuple<int, int>, int> img_to_cloud;
      int row = rows - 1;
      ...
      for (auto &element : points) {
        ...
        img_to_cloud.emplace(std::make_tuple(real_row, real_col), idx);
        ++idx;
      ...
      for (auto && [ k, v ] : img_to_cloud) {
        auto && [ row, col ] = k;

        points[v].rgb[0] = blurred(row, col)[2];
        points[v].rgb[1] = blurred(row, col)[1];
        points[v].rgb[2] = blurred(row, col)[0];
      }
