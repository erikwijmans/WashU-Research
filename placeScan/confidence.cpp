...
void confidence::findMostConfidentPlacements(
    std::vector<confidence::moreInfo> &info) {
  ...
  confidence::findLocalMinima(scoreInfo, localMins, pointFileNames,
                              rotationFileNames, zerosFileNames);
...
void confidence::loadInPlacement(const std::string &scanName,
                                 std::vector<confidence::moreInfo> &scoreVec,
                                 const int scanNum) {
  const std::string placementName =
      FLAGS_preDone + scanName.substr(scanName.find("_") - 3, 3) +
...
void confidence::findLocalMinima(
    const std::vector<confidence::moreInfo> &scoreInfo,
    std::vector<int> &localMins, std::vector<std::string> &pointFileNames,
    std::vector<std::string> &rotationFileNames,
    std::vector<std::string> &zerosFileNames) {
  ...
  for (int i = 0; i < scoreInfo.size(); ++i) {
    const std::string scanName =
        FLAGS_dmFolder + pointFileNames[scoreInfo[i].scanNum];
    const std::string rotName =
        FLAGS_rotFolder + rotationFileNames[scoreInfo[i].scanNum];
    const std::string zeroName =
        FLAGS_zerosFolder + zerosFileNames[scoreInfo[i].scanNum];
