/*
This program takes a video in the form of a series of still and its IMU file and
stitches a panorama
out of the stills based solely on the rotation information.

The stitching is done by transforming the corners of an image into world
coordinates and then projecting those
world coordinates onto the panaorama by transforming cartesian coordinates to
spherical coordinates and scaling them

Once the corners have been found, the largest possible square those corners can
make up is calculated.  Then every pixel on
...
int main(int argc, char **argv) {
  // Moto X K
  K = (Mat_<float>(3, 3) << 3867.214290 / 3.25, 0, 1220.329623 / 3.25, 0,
       3850.296435 / 3.25, 2077.400229 / 3.25, 0, 0, 1);
...
Mat readInRotation(ifstream &file, float timeSamp) {
  ...
  if (file && !file.eof()) {
    ...
    file.read(reinterpret_cast<char *>(&rot_matrix.at<float>(0, 0)),
              sizeof(float) * 3);
    file.read(reinterpret_cast<char *>(&rot_matrix.at<float>(1, 0)),
              sizeof(float) * 3);
    file.read(reinterpret_cast<char *>(&rot_matrix.at<float>(2, 0)),
              sizeof(float) * 3);
...
void image_coords_to_pano_coords(float *img_coords, float *pano_coords,
                                 Mat &R) {
...
void pano_coords_to_image_coords(float *pano_coords, float *img_coords,
  ...
  img_coords[0] =
      720 - 1 - image_coords.at<float>(0) / image_coords.at<float>(2);
