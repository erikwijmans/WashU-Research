...
/*
Takes a path and solves for what the correct path should by making some
assumptions.
For least squares regression method, it assumes that the start and end points
must be the same.  It also assumes
that the acceleration and velocity at any point must be the same in the input
and output paths.  Also, it locks the start point to (0,0)

For linear programming (which is solved with Gurobi).  It assumes the start and
end points must be the same.  It also assumes that the
...
void printPath(const char *name, VectorXf &x) {
  ...
  pathWriter << "<!DOCTYPE html>" << endl
             << "<html>" << endl
             << "<body>" << endl
             << endl
             << "<svg height=\"1000\" width=\"1000\">" << endl
             << "<image "
                "xlink:href=\"file:/home/erik/Projects/c++/PathRectifier/"
                "Bryan+Jolley.png\""
             << "x=\"-50\" y=\"-300\" width=\"1000\" height=\"1000\" />" << endl
             << endl
  ...
  pathWriter << "\""
             << " style=\"fill:none;stroke:black;stroke-width:2\" />" << endl
             << "Sorry, your browser does not support inline SVG." << endl
             << "</svg>" << endl
             << endl
             << "</body>" << endl
             << "</html>" << flush;
...
VectorXf lPSolver(VectorXf &b) {
  try {
    ...
    for (int i = 0; i < 2 * F - 4; ++i) {
      model.addConstr(-1 * dVarList[i] <= xyVarList[i] - 2 * xyVarList[i + 2] +
                                              xyVarList[i + 4] -
                                              b(4 + 2 * F - 2 + i));
      model.addConstr(xyVarList[i] - 2 * xyVarList[i + 2] + xyVarList[i + 4] -
                          b(4 + 2 * F - 2 + i) <=
                      dVarList[i]);
    }
